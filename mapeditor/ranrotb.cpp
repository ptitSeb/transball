...
*  Note: it has not been tested with intel EMT64 machines (but it should *
*        work as well.                                                   *
*  The changes made are simply substitute all "long" keywords by "int".  *
*************************************************************************/
...
inline unsigned int
_lrotl(unsigned int value, int shift)
{
...
// constructor:
TRanrotBGenerator::TRanrotBGenerator(int seed) {
  // detect computer architecture
  union {double f; unsigned int i[2];} convert;
  convert.f = 1.0;
  if (convert.i[1] == 0x3FF00000) Architecture = TA_LITTLE_ENDIAN;
  else if (convert.i[0] == 0x3FF00000) Architecture = TA_BIG_ENDIAN;
  else Architecture = TA_NON_IEEE;
  RandomInit(seed);
}
...
double TRanrotBGenerator::Random() {
  ...
  if (randbuffer[p1] == randbufcopy[0] &&
    memcmp(randbuffer, randbufcopy+KK-p1, KK*sizeof(int)) == 0) {
      // self-test failed
      if ((p2 + KK - p1) % KK != JJ) {
...
void TRanrotBGenerator::RandomInit (int seed) {
  ...
  union {
    double randp1;
    unsigned int randbits[2];};
  randp1 = 1.5;
    
  if (Architecture == TA_LITTLE_ENDIAN) assert(randbits[1]==0x3FF80000); // check that IEEE double precision float format used
  if (Architecture == TA_BIG_ENDIAN) assert(randbits[0]==0x3FF80000); // check that IEEE double precision float format used

  // initialize pointers to circular buffer
  p1 = 0;  p2 = JJ;
  // store state for self-test
  memcpy (randbufcopy, randbuffer, KK*sizeof(int));
  memcpy (randbufcopy+KK, randbuffer, KK*sizeof(int));
  // randomize some more
