...
/*
  ...
  Log:
  16-april-2006: Added the "randomize" method
  13-april-2006: Implementeg the "SetObj(int i,T *O)" method, that was declared but not implemented!
  19-march-2006: Added the "ExtractAll" method (extracts all the elements of a list without freeing them)
  9-march-2006: Added the "ExtractRandom" method
  22-octobar-2005: Base version (from an early version developed some time around 1998)
...
public:
	...
	List<T>(List<T> &l) {list=0;act=0;top=0;original=true;Copy(l);};
	...
	{
		List<T> tmp;
		while(!EmptyP()) tmp.Add(ExtractIni());
		while(!tmp.EmptyP()) Add(tmp.ExtractRandom());
	} /* Randomize */ 
	...
	bool integrity_test(void)
	{
		...
		pos=list;
		while(pos!=0) {
			if (l.MemberRefP(pos)) return false;
			l.Add(pos);
			pos=pos->Getnext();
		} /* while */ 

		if (!l.MemberRefP(top)) return false;

		while(!l.EmptyP()) l.ExtractIni();

		if (list!=0) {
			if (top==0) return false;
			if (top->Getnext()!=0) return false;
		} else {
			if (top!=0) return false;
		} /* if */ 
...
public:
	...
	~LLink() {delete []obj;
		 	  if (next!=0) delete next;};
	inline LLink *Getnext() {return next;};
	inline void Setnext(LLink *n) {next=n;};
	inline char *GetObj() {return obj;};
	inline void SetObj(char *o) {obj=o;};
...
public:
	...
	void Instance(List<char> &l) {list=l.list;act=list;top=l.top;original=false;};
	void Rewind(void) {act=list;};
	void Forward(void) {act=top;};
	void Next(void) {
		if (act!=0) act=act->Getnext();
	...
	char *GetObj(void) {return act->GetObj();};
	void SetObj(char *o) {act->SetObj(o);};
	...
	LLink<char> *GetPos(void) {return act;};
	void SetPos(LLink<char> *p) {act=p;};
	bool EmptyP() {return list==0;};
	bool EndP() {return act==0;};
	bool LastP() {return act==top;};
	bool BeginP() {return act==list;};
	...
	int Position(char *o) {
		...
		tmp=list;
		while(tmp!=0) {
			if (strcmp(tmp->GetObj(),o)==0) return i;
			i++;
			tmp=tmp->Getnext();
		} /* while */ 
		return -1;
