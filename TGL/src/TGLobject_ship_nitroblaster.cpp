...
TGLobject_ship_nitroblaster::TGLobject_ship_nitroblaster(float x,float y) : TGLobject_ship(x,y)
{
	m_scale=0.45f;
	m_thrusting=false;
...
bool TGLobject_ship_nitroblaster::cycle(VirtualController *k,TGLmap *map,GLTManager *GLTM,SFXManager *SFXM,int sfx_volume)
{
	...
	if (m_nitrothrusting) {
		if ((m_cycle%2)==0) map->add_auxiliary_back_object(new TGLobject_FX_particle(get_x(),get_y(),rand()%60,0,0,1,false,0.2f,0,0.5f,1.5f,50,GLTM->get("objects/smoke")));
	} // if
...
void TGLobject_ship_nitroblaster::draw(GLTManager *GLTM)
{
	if (m_last_mask==0) m_last_mask=GLTM->get("objects/ship-nblaster-1");

	if (!m_thrusting && !m_nitrothrusting) {
		m_last_tile=GLTM->get("objects/ship-nblaster-1");
	} // if 
	if (m_thrusting && !m_nitrothrusting) {
		if (((m_cycle/4)%2)==0) m_last_tile=GLTM->get("objects/ship-nblaster-2");
						   else m_last_tile=GLTM->get("objects/ship-nblaster-3");
	} // if 
	if (!m_thrusting && m_nitrothrusting) {
		if (((m_cycle/4)%2)==0) m_last_tile=GLTM->get("objects/ship-nblaster-4");
						   else m_last_tile=GLTM->get("objects/ship-nblaster-5");
	} // if 
