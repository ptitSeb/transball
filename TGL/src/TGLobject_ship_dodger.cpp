...
bool TGLobject_ship_dodger::cycle(VirtualController *k,TGLmap *map,GLTManager *GLTM,SFXManager *SFXM,int sfx_volume)
{
	...
	if (k->m_button[0] && !k->m_old_button[0] && m_fuel>40) {
		...
		while(a<0) a+=360;
		while(a>=360) a-=360;
		bullet=new TGLobject_bullet(float(get_x()+(cos_table[a]*12)),float(get_y()+(sin_table[a]*12)),m_angle,6,1,GLTM->get("objects/bullet-green"),this);
...
void TGLobject_ship_dodger::draw(GLTManager *GLTM)
{
	char sp_name[32];
	int sprite=0;

	if (m_last_mask==0) m_last_mask=GLTM->get("objects/ship-dodger-1");
	...
	sprintf(sp_name,"objects/ship-dodger-%i",sprite);
