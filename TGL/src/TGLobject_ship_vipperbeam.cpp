...
TGLobject_ship_vipperbeam::TGLobject_ship_vipperbeam(float x,float y) : TGLobject_ship(x,y)
{
	m_scale=0.45f;
	m_thrusting=false;
	m_ball=0;
...
bool TGLobject_ship_vipperbeam::cycle(VirtualController *k,TGLmap *map,GLTManager *GLTM,SFXManager *SFXM,int sfx_volume)
{
	...
	if (k->m_button[0] && m_fuel>=2) {
		...
		// laser:
		{
			...
			for(i=0;i<640/4 && !collision;i++) {
				o=new TGLobject_FX_particle(lx,ly,a+90,0,0,0,false,
											((i+(m_cycle/2))%4)*0.2f+0.6f,((i+(m_cycle/2))%4)*0.2f+0.6f,0.33f,0.33f,1,GLTM->get_smooth("objects/vipper-laser"));
				...
				if (map->collision(o,0,0,0)) {
...

					{
						int j;
						int range=100;
						for(j=0;j<4;j++) map->add_auxiliary_front_object(new TGLobject_FX_particle(lx,ly,rand()%60,float((rand()%(range*2+1))-range)/256.0f,float((rand()%(range*2+1))-range)/256.0f,4,true,0.5f,0.5f,1,1,0.5f,0.5f,1.0f,0,0.25f,0.25f,50,GLTM->get("objects/particle1")));
					} // if
		...
		if (m_laser_channel==-1) {
			m_laser_channel=Sound_play_continuous(SFXM->get("sfx/laser"),sfx_volume);
		} // if 
	} else {
		if (m_laser_channel!=-1) {
			Mix_HaltChannel(m_laser_channel);
...
void TGLobject_ship_vipperbeam::draw(GLTManager *GLTM)
{
	if (m_last_mask==0) m_last_mask=GLTM->get("objects/ship-vbeam-1");

	if (m_thrusting) {
		if (((m_cycle/4)%2)==0) m_last_tile=GLTM->get("objects/ship-vbeam-2");
						   else m_last_tile=GLTM->get("objects/ship-vbeam-3");
	} else {
		m_last_tile=GLTM->get("objects/ship-vbeam-1");
	} // if 
