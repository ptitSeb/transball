...
TGLobject_fastcannon_up::TGLobject_fastcannon_up(float x,float y) : TGLobject_enemy(x,y)
{
	m_state=0;
	m_hitpoints=8;
...
bool TGLobject_fastcannon_up::cycle(VirtualController *k,class TGLmap *map,GLTManager *GLTM,SFXManager *SFXM,int sfx_volume)
{
	...
	case 1: ship=map->object_exists("TGLobject_ship",get_x()-16,get_y(),get_x()+48,get_y()+240);
			if (ship!=0) {
				// fire
				map->add_object_back(new TGLobject_bullet(get_x()+17,get_y()+30,180,5,1,GLTM->get("objects/bullet-grey"),this));
				m_state=64;
...
void TGLobject_fastcannon_up::draw(GLTManager *GLTM)
{
	m_last_tile=GLTM->get("objects/fastcannon-techno-up1");
