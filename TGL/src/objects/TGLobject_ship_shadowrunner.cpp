...
bool TGLobject_ship_shadowrunner::cycle(VirtualController *k,TGLmap *map,GLTManager *GLTM,SFXManager *SFXM,int sfx_volume)
{
	...
	if (map->collision(this,0,0,0)) {
		if (!map->collision_with_foreground(this,0,0,0)) {
			...
			if (obj->is_a("TGLobject_soft_bullet")) {
				...
				while(a<0) a+=360;
				while(a>=360) a-=360;
				m_speed_x+=float(cos_degree(a)*64.0)/256.0f;
				m_speed_y+=float(sin_degree(a)*64.0)/256.0f;
			} else {
	...
	if (k->m_button[0] && !k->m_old_button[0] && m_fuel>40) {
		...
		while(a<0) a+=360;
		while(a>=360) a-=360;
		bullet=new TGLobject_bullet(float(get_x()+(cos_degree(a)*12)),float(get_y()+(sin_degree(a)*12)),0,m_angle,6,1,GLTM->get("objects/bullet-purple"),this);
	...
	if (k->m_joystick[VC_UP] && m_fuel>0) {
		...
		while(a<0) a+=360;
		while(a>=360) a-=360;
		m_speed_x+=float(cos_degree(a)*24.0)/256.0f;
		m_speed_y+=float(sin_degree(a)*24.0)/256.0f;
		m_thrusting=true;
		m_fuel--;
	} // if 
	...
	if (k->m_joystick[VC_DOWN] && m_fuel>0) {
		...
		while(a<0) a+=360;
		while(a>=360) a-=360;
		m_speed_x-=float(cos_degree(a)*8.0)/256.0f;
		m_speed_y-=float(sin_degree(a)*8.0)/256.0f;
		m_backthrusting=true;
		m_fuel--;
	} // if 
	...
	if (m_thrusting || m_backthrusting) {
		if (m_thrust_channel==-1) {
...
void TGLobject_ship_shadowrunner::draw(GLTManager *GLTM)
{
	if (m_last_mask==0) m_last_mask=GLTM->get("objects/ship-srunner-1");

	if (!m_thrusting && !m_backthrusting) {
		m_last_tile=GLTM->get("objects/ship-srunner-1");
	} // if 
	if (m_thrusting && !m_backthrusting) {
		if (((m_cycle/4)%2)==0) m_last_tile=GLTM->get("objects/ship-srunner-2");
						   else m_last_tile=GLTM->get("objects/ship-srunner-3");
	} // if 
	if (!m_thrusting && m_backthrusting) {
		if (((m_cycle/4)%2)==0) m_last_tile=GLTM->get("objects/ship-srunner-4");
						   else m_last_tile=GLTM->get("objects/ship-srunner-5");
	} // if 
	if (m_thrusting && m_backthrusting) {
		if (((m_cycle/4)%2)==0) m_last_tile=GLTM->get("objects/ship-srunner-6");
						   else m_last_tile=GLTM->get("objects/ship-srunner-7");
	} // if 

	if (m_last_tile!=0) m_last_tile->draw(m_x,m_y,0,float(m_angle),m_scale);
} /* TGLobject_ship_shadowrunner::draw */ 


bool TGLobject_ship_shadowrunner::is_a(Symbol *c)
{
	if (c->cmp("TGLobject_ship_shadowrunner")) return true;
	return TGLobject_ship::is_a(c);
} /* TGLobject_ship_shadowrunner::is_a */ 


bool TGLobject_ship_shadowrunner::is_a(char *c)
{
	bool retval;
...
const char *TGLobject_ship_shadowrunner::get_class(void)
{
	return "TGLobject_ship_shadowrunner";
} /* TGLobject_ship_shadowrunner:get_class */ 
