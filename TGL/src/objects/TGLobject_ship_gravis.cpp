...
TGLobject_ship_gravis::TGLobject_ship_gravis(float x,float y,int initial_fuel) : TGLobject_ship(x,y,initial_fuel)
{
	m_scale=0.45f;
	m_thrusting=false;
	m_ball=0;
...
bool TGLobject_ship_gravis::cycle(VirtualController *k,TGLmap *map,GLTManager *GLTM,SFXManager *SFXM,int sfx_volume)
{
	...
	if (k->m_button[0] && !k->m_old_button[0] && m_fuel>40) {
		...
		while(a<0) a+=360;
		while(a>=360) a-=360;
		bullet=new TGLobject_bullet(float(get_x()+(cos_degree(a)*12)),float(get_y()+(sin_degree(a)*12)),0,m_angle,6,1,GLTM->get("objects/bullet-darkblue"),this);
	...
	if (m_thrusting) {
		...
		while(a<0) a+=360;
		while(a>=360) a-=360;
		if ((m_cycle%4)==0) map->add_auxiliary_back_object(new TGLobject_FX_particle(float(get_x()+(cos_degree(a)*8)-(sin_degree(a)*8)),float(get_y()+(sin_degree(a)*8)+(cos_degree(a)*8)),rand()%60,0,0,1,false,0.125f,0,0.25f,0.75f,50,GLTM->get("objects/smoke")));
		if ((m_cycle%4)==0) map->add_auxiliary_back_object(new TGLobject_FX_particle(float(get_x()-(cos_degree(a)*8)-(sin_degree(a)*8)),float(get_y()-(sin_degree(a)*8)+(cos_degree(a)*8)),rand()%60,0,0,1,false,0.125f,0,0.25f,0.75f,50,GLTM->get("objects/smoke")));
