...
TGLobject_ship_xterminator::TGLobject_ship_xterminator(float x,float y,int initial_fuel) : TGLobject_ship(x,y,initial_fuel)
{
	m_scale=0.45f;
	m_thrusting=false;
	m_ball=0;
...
bool TGLobject_ship_xterminator::cycle(VirtualController *k,TGLmap *map,GLTManager *GLTM,SFXManager *SFXM,int sfx_volume)
{
	...
	if (k->m_button[0] && !k->m_old_button[0] && m_fuel>96) {
		TGLobject *bullet;
		int a=m_angle-90;
		while(a<0) a+=360;
		while(a>=360) a-=360;
		bullet=new TGLobject_bullet_missile(float(get_x()+(cos_degree(a)*8)),float(get_y()+(sin_degree(a)*8)),0,m_angle,4,4,GLTM->get("objects/bullet-grey-missile1"),GLTM->get("objects/bullet-grey-missile2"),this);
...
void TGLobject_ship_xterminator::draw(GLTManager *GLTM)
{
	if (m_last_mask==0) m_last_mask=GLTM->get("objects/ship-xterminator-1");

	if (m_thrusting) {
		if (((m_cycle/4)%2)==0) m_last_tile=GLTM->get("objects/ship-xterminator-2");
						   else m_last_tile=GLTM->get("objects/ship-xterminator-3");
	} else {
		m_last_tile=GLTM->get("objects/ship-xterminator-1");
	} // if 

	if (m_last_tile!=0) m_last_tile->draw(m_x,m_y,0,float(m_angle),m_scale);
} /* TGLobject_ship_xterminator::draw */ 


bool TGLobject_ship_xterminator::is_a(Symbol *c)
{
	if (c->cmp("TGLobject_ship_xterminator")) return true;
	return TGLobject_ship::is_a(c);
} /* TGLobject_ship_xterminator::is_a */ 


bool TGLobject_ship_xterminator::is_a(char *c)
{
	bool retval;
...
const char *TGLobject_ship_xterminator::get_class(void)
{
	return "TGLobject_ship_xterminator";
