...
TGLobject_laser_up::TGLobject_laser_up(float x,float y,int ao) : TGLobject_enemy(x,y,ao)
{
	m_hitpoints=16;
} /* TGLobject_laser_up::TGLobject_laser_up */ 
...
bool TGLobject_laser_up::cycle(VirtualController *k,class TGLmap *map,GLTManager *GLTM,SFXManager *SFXM,int sfx_volume)
{
	m_cycle++;
...
} /* TGLobject_laser_up::draw */ 


bool TGLobject_laser_up::is_a(Symbol *c)
{
	if (c->cmp("TGLobject_laser_up")) return true;
	return TGLobject_enemy::is_a(c);
} /* TGLobject_laser_up::is_a */ 


bool TGLobject_laser_up::is_a(char *c)
{
	bool retval;
...
const char *TGLobject_laser_up::get_class(void)
{
	return "TGLobject_laser_up";
} /* TGLobject_laser_up::get_class */ 
