...
bool TGLobject_ship_c_harpoon::cycle(VirtualController *k,TGLmap *map,GLTManager *GLTM,SFXManager *SFXM,int sfx_volume)
{
	...
	if (k->m_button[0] && !k->m_old_button[0] && m_fuel>32) {
		...
		while(a<0) a+=360;
		while(a>=360) a-=360;
		bullet=new TGLobject_bullet(float(get_x()+(cos_degree(a)*12)),float(get_y()+(sin_degree(a)*12)),0,m_angle,5,1,GLTM->get("objects/bullet-red"),this);
...
//		Sound_play(SFXM->get("sfx/shipshot"),sfx_volume);
	...
	if (m_thrusting) {
		...
		if ((m_cycle%4)==0) map->add_auxiliary_back_object(new TGLobject_FX_particle(float(get_x()+(cos_degree(a)*11)),float(get_y()+(sin_degree(a)*11)),rand()%60,0,0,1,false,0.125f,0,0.375f,1.0f,50,GLTM->get("objects/smoke")));
		if ((m_cycle%4)==0) map->add_auxiliary_back_object(new TGLobject_FX_particle(float(get_x()-(cos_degree(a)*11)),float(get_y()-(sin_degree(a)*11)),rand()%60,0,0,1,false,0.125f,0,0.375f,1.0f,50,GLTM->get("objects/smoke")));
