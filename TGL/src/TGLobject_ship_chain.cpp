...
void TGLobject_ship_chain::chain_cycle(void)
{
	...
	if (m_previous_join_x==m_next_join_x &&
		...
		desired_next_join_x=m_x+float((CHAIN_LINK_LENGTH/2)*cos_table[90-m_angle]);
		desired_next_join_y=m_x+float((CHAIN_LINK_LENGTH/2)*sin_table[90-m_angle]);
	} else {
		...
		desired_x=center_x+float((-CHAIN_LINK_LENGTH/2)*cos_table[desired_angle]);
		desired_y=center_y+float((-CHAIN_LINK_LENGTH/2)*sin_table[desired_angle]);
		desired_next_join_x=center_x+float((CHAIN_LINK_LENGTH/2)*cos_table[desired_angle]);
		desired_next_join_y=center_y+float((CHAIN_LINK_LENGTH/2)*sin_table[desired_angle]);
...
void TGLobject_ship_chain::draw(GLTManager *GLTM)
{
	if (m_last_mask==0) m_last_mask=GLTM->get("objects/ship-harpoon-4");
	if (m_last_tile==0) m_last_tile=GLTM->get("objects/ship-harpoon-4");
