...
bool TGLobject_ship_shadowrunner::cycle(VirtualController *k,TGLmap *map,GLTManager *GLTM,SFXManager *SFXM,int sfx_volume)
{
	...
	if (map->collision(this,0,0,0)) {
		if (!map->collision_with_foreground(this,0,0,0)) {
			...
			if (obj->is_a("TGLobject_soft_bullet")) {
				...
				while(a<0) a+=360;
				while(a>=360) a-=360;
				m_speed_x+=float(cos_table[a]*64.0)/256.0f;
				m_speed_y+=float(sin_table[a]*64.0)/256.0f;
			} else {
	...
	if (k->m_button[0] && !k->m_old_button[0] && m_fuel>40) {
		...
		while(a<0) a+=360;
		while(a>=360) a-=360;
		bullet=new TGLobject_bullet(float(get_x()+(cos_table[a]*12)),float(get_y()+(sin_table[a]*12)),0,m_angle,6,1,GLTM->get("objects/bullet-purple"),this);
	...
	if (k->m_joystick[VC_UP] && m_fuel>0) {
		...
		while(a<0) a+=360;
		while(a>=360) a-=360;
		m_speed_x+=float(cos_table[a]*24.0)/256.0f;
		m_speed_y+=float(sin_table[a]*24.0)/256.0f;
		m_thrusting=true;
		m_fuel--;
	} // if 
	...
	if (k->m_joystick[VC_DOWN] && m_fuel>0) {
		...
		while(a<0) a+=360;
		while(a>=360) a-=360;
		m_speed_x-=float(cos_table[a]*8.0)/256.0f;
		m_speed_y-=float(sin_table[a]*8.0)/256.0f;
