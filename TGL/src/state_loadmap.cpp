...
int TGLapp::loadmap_cycle(KEYBOARDSTATE *k)
{
	...
	if (m_mb_recheckfiles) {
		...
		// Load the map filenames:
		{
			...
			for(i=0;i<m_mb_m_names.Length();i++) {
				char *tmp;

				tmp=new char[strlen(m_mb_m_names[i])+9];
				sprintf(tmp,"%s/%s",m_mb_current_path,m_mb_m_names[i]);
				m_mb_m_fullnames.Add(tmp);
			} // for
		}
		...
		if (mouse_x>=20 && mouse_x<600) {
			if (mouse_y>=40) {
				...
				if (selected>=0 && selected<(m_mb_m_names.Length()+m_mb_folders_names.Length()-m_mb_first_m) && selected<MAPBROWSER_MPERPAGE) {
					...
					if (button==1) m_mb_m_selected=m_mb_first_m+selected;
				} // if 
			} // if
		} // if 
	...
	if (m_mb_m_selected==-1) m_mb_select_button->m_enabled=false;
						  else m_mb_select_button->m_enabled=true;
