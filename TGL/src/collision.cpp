...
bool collision_with_point(SDL_Surface *s1,float x1,float y1,int hot_x1,int hot_y1,int a1,float scale1,float x2,float y2)
{
	...
	// Create a temprorary surface where to perform the collision check:
	{
						   ...
						   else	obj1_sfc=rotozoomSurface(s1,-a1,scale1,0);
		{
			...
			minx=min(r1.x,int(x2+0));
			miny=min(r1.y,int(y2+0));
			maxx=max(r1.x+r1.w,int(x2+1));
			maxy=max(r1.y+r1.h,int(y2+1));
...

			{
				...
				common_x2=min(r1.x+r1.w,int(x2+1));

				common_y1=max(r1.y,int(y2+0));
